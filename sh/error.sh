...
if [ "$IFTTT_MAKER_KEY" != "" ]; then
	...
	curl -X POST -H "Content-Type: application/json" -d "{\"value1\":\"Downloading $FNAME failed. Error: $DLERR\"}" https://maker.ifttt.com/trigger/$IFTTT_MAKER_EVENT/with/key/$IFTTT_MAKER_KEY
fi
